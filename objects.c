...
typedef struct{
   ...
   float Ka[4], Kd[4], Ks[4], Ke[4], Ns; //  Colors and shininess
...
static char* readline(FILE* f){
   ...
   while((ch = fgetc(f)) != EOF){
      ...
      if(k >= linelen){
         ...
         if(!line){
            throw_error("Out of memory in readline\n");
         }
      }
      //  End of Line
      if(CRLF(ch)){
...
static void readfloat(char* line, int n, float x[]){
   ...
   for (i = 0; i < n; i++){
      ...
      if(!str){
         throw_error("Premature EOL reading %d floats\n", n);
      }
      if(sscanf(str, "%f", x + i) != 1){
         throw_error("Error reading float %d\n", i);
      }
   }
}
...
static void LoadMaterial(const char* file){
   ...
   while((line = readline(f))){
      ...
      if((str = readstr(line, "newmtl"))){
         ...
         if(!mtl[k].name){
            throw_error("Cannot allocate %d for name\n", l + 1);
         }
...
static void SetMaterial(const char* name){
   ...
   for(k = 0; k < Nmtl; k++){
      if(!strcmp(mtl[k].name, name)){
         //  Set material colors
         glMaterialfv(GL_FRONT, GL_AMBIENT  , mtl[k].Ka);
         glMaterialfv(GL_FRONT, GL_DIFFUSE  , mtl[k].Kd);
         glMaterialfv(GL_FRONT, GL_SPECULAR , mtl[k].Ks);
         glMaterialfv(GL_FRONT, GL_EMISSION , mtl[k].Ke);
         glMaterialf(GL_FRONT, GL_SHININESS, mtl[k].Ns);
      ...
      //  Read and draw facets
         ...
         while((str = getword(&line))){
            ...
            if(sscanf(str, "%d/%d/%d", &Kv, &Kt, &Kn) == 3){
               if(Kv < 0 || Kv > Nv / 3){
                  throw_error("Vertex %d out of range 1-%d\n", Kv, Nv / 3);
               }
               if(Kn < 0 || Kn > Nn / 3){
                  throw_error("Normal %d out of range 1-%d\n", Kn, Nn / 3);
               }
               if(Kt < 0 || Kt > Nt / 2){
                  throw_error("Texture %d out of range 1-%d\n", Kt, Nt / 2);
               }
            }
            ...
            //  This is an error
               ...
               throw_error("Invalid facet %s\n", str);
            }
            //  Draw vectors
            if(Kt){
               glTexCoord2fv(T + 2 * (Kt - 1));
            }
            if(Kn){
               glNormal3fv(N + 3 * (Kn - 1));
            }
            if(Kv){
